...
class DOAJXmlFilter extends NativeExportFilter {
	...
	function &process(&$pubObjects) {
		...
		foreach($pubObjects as $pubObject) {
			...
			// Language
			$language = AppLocale::get3LetterIsoFromLocale($currentLocale);
			...
			// documentType
			$type = $publication->getLocalizedData('type', $currentLocale);
			...
			if (array_key_exists($currentLocale, $articleTitles)) {
				$titleInArticleLocale = $articleTitles[$currentLocale];
				unset($articleTitles[$currentLocale]);
				$articleTitles = array_merge(array($currentLocale => $titleInArticleLocale), $articleTitles);
			}
			...
			if (array_key_exists($currentLocale, $articleAbstracts)) {
				$abstractInArticleLocale = $articleAbstracts[$currentLocale];
				unset($articleAbstracts[$currentLocale]);
				$articleAbstracts = array_merge(array($currentLocale => $abstractInArticleLocale), $articleAbstracts);
			}
			...
			if (array_key_exists($currentLocale, $articleKeywords)) {
				$keywordsInArticleLocale = $articleKeywords[$currentLocale];
				unset($articleKeywords[$currentLocale]);
				$articleKeywords = array_merge(array($currentLocale => $keywordsInArticleLocale), $articleKeywords);
			}
